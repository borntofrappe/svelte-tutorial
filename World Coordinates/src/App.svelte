...
<svg 
	...
	<g>
		...
		{#await promise}
			<text out:fly={{duration: 250}} x={dimensions.width / 2} y={dimensions.height / 2} text-anchor="middle" dominant-baseline="middle" font-size="32">Fetching data</text>
		{:then data}
		...
		{:catch error}
			<g in:fly={{delay: 500}}>
				<text x={dimensions.width / 2} y={dimensions.height / 2 - 24} text-anchor="middle" dominant-baseline="middle" font-size="32">Error</text>
				<text x={dimensions.width / 2} y={dimensions.height / 2 + 18} text-anchor="middle" dominant-baseline="middle" font-size="20">{error}</text>
			</g>
		{/await}
	</g>
</svg>
...
{#if coordinates.length > 0}
	<ul>
		{#each coordinates.map(({long, lat}) => ([ formatCoordinates(long), formatCoordinates(lat)])) as [long, lat], i}
			<li>
				...
				<button on:click="{() => {
													coordinates = [...coordinates.slice(0, i), ...coordinates.slice(i + 1)]
													}}">
					Remove
				</button>
			</li>
	...
	<button on:click="{() => {
											copy(`[${coordinates.map(({ long, lat }) => `[${long}, ${lat}]`).join(', ')}]`)
											notice = 'Copied all'
										}}">
		Copy all
	</button>

	<button on:click="{() => coordinates = []}">
		Remove all
	</button>
{/if}
