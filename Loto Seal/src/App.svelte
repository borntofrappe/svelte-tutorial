<script>
	...
	let seal;
	let mouse;
	$: isFound = seal && mouse && seal.column === mouse.column && seal.row === mouse.row;
	
	function raise(node, { delay = 0, duration = 1000}) {
		return {
			delay,
			duration,
...
{#if isFound}
	<svg in:raise aria-hidden="true" xmlns="http://www.w3.org/2000/svg"  style="position: absolute; left: {seal.column * resolution}px; top: {(seal.row - 1) * resolution}px" viewBox="0 -0.5 10 10" width="{resolution}" height="{resolution}" shape-rendering="crispEdges">
		<path stroke="#0c1019" d="M4 0h4M2 1h2M8 1h1M1 2h1M9 2h1M1 3h1M9 3h1M0 4h1M9 4h1M0 5h1M3 5h1M8 5h2M0 6h1M8 6h2M0 7h2M6 7h3M1 8h7M2 9h4" />
		<path stroke="#c38900" d="M4 1h1M6 1h2M6 2h3M2 3h2M5 3h1M8 3h1M4 4h1M7 4h1M1 5h1M5 5h1M7 5h1M1 6h1M5 6h1M7 6h1M2 7h2M5 7h1" />
		<path stroke="#ffb700" d="M5 1h1M2 2h4M4 3h1M6 3h2M1 4h3M5 4h2M8 4h1M2 5h1M4 5h1M6 5h1M2 6h3M6 6h1M4 7h1" />
	</svg>
{/if}
...
<path stroke="#081113" d="M29 14h1" />
	...
	{:else if mouse}
		<p in:fade>
				Try {Math.abs(seal.row - mouse.row)} to the {seal.row - mouse.row > 0 ? 'south' : 'north'} and {Math.abs(seal.column - mouse.column)} to the {seal.column - mouse.column > 0 ? 'east' : 'west'}</p>
	{:else}
		<p in:fade>
			The <strong>Loto Seal</strong> is somewhere in this page. It's hidden in a tile {resolution} pixels wide and tall, and it answers to a click event.
		</p>
	{/if}
</main>
