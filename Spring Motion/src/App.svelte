<script>
	...
	*/
	...
	$: d = `${path.map(([x, y], i) => i === 0 ? `M ${x} ${y}` : `L ${x} ${y}`).join("")}`
...
<svg on:mousemove={handleMousemove} on:click={() => path = []}>
	...
	<path opacity="0.4" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="5" stroke-dasharray="10 20" d="{d}" />

	<!-- bug icon translated and rotated with the springs' values -->
	<g transform="translate({$coords.x} {$coords.y}) rotate({$angle})">
			<g transform="scale(0.5)" stroke="currentColor" stroke-linejoin="round" stroke-linecap="round">
