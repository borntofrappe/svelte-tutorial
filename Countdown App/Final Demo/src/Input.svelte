<script>
  ...
  function handleSubmit() {
    const { h, m, s } = time;
    const seconds = [s, m, h].reduce(
      (acc, curr, i) => acc + parseInt(curr, 10) * 60 ** i,
      0
    );
    dispatch('countdown', seconds);
  }
...
<header in:fly={{ y: -5 }}>
  <h1 class:translucent={!value}>{@html timeMarkup}</h1>
  <button disabled={!value} on:click={handlePop}>
...
<Keypad bind:value on:submit={handleSubmit} />
