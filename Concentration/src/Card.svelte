...
<article
  class:flip={card.isFlipped && !card.isPaired}
  class:paired={card.isPaired}
>
  <div class="face front">
    <h1
      style="color: {card.value.includes('♥') || card.value.includes('♦')
        ? 'hsl(0, 70%, 50%)'
        : 'hsl(0, 50%, 5%)'}"
    >
      {card.value}
  ...
  <button disabled={card.isPaired} aria-label="Flip card" on:click={flip} />
</article>
