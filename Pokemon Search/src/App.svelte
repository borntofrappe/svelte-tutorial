...
<div>	
	<form on:submit|preventDefault={handleSubmit}>
		<label>
			...
			<input required type="number" bind:value={number} min="1" max="{pokemon}" placeholder="Search by number" />
		</label>
	...
	{#if promise}
		{#await promise then {name, sprites, types, stats}}
			<Entry on:favorite={handleFavorite} {...{name, sprites, types, stats, isFavorite: favorites[fetchedNumber]}} />
		{:catch error}
			<p>
				{error}
			</p>
