<script>
  ...
  $: if (colors) {
    dataColors.children = Object.entries(colors).map(([color, value]) => ({
      color,
      value,
      id: `${id}-${color}`,
    }));
...
<p>
  <span class="highlight">{userId}</span> created
  <a
    class="highlight"
    href="https://blockbuilder.org/{userId}/{id}"
    target="_blank">{description}</a
  >
  on <time datetime={date}>{formatDate(date)}</time>, making use of
  <strong>{dataColors.children.length}</strong>
  {dataColors.children.length === 1 ? 'color' : 'colors'}.
</p>
