...
<div>
	...
	<svg viewBox="-0.5 -0.5 {totalWidth + 1} {totalHeight + 1}">
		<defs>
			<pattern patternUnits="userSpaceOnUse" id="pattern" width={cellSize} height={cellSize} viewBox="0 0 10 10">
				<rect width="10" height="10" fill="none" stroke="currentColor" stroke-width="1" />
			</pattern>
		</defs>
		<g>
			<g fill="currentColor" font-size="{cellSize / 2}" text-anchor="middle" dominant-baseline="middle" transform="translate({padding / 2} {padding / 2})">
					...
					<g>
						...
						{#each Array(columns) as c, i}
							<g transform="translate({i * cellSize} 0)">
								...
								<text font-size={cellSize / 2} fill="currentColor" stroke="none" text-anchor="middle" x={cellSize / 2} y={padding - 2}>{i}</text>
							</g>
						{/each}
						</g>
						...
						{#each Array(rows) as r, i}
							<g transform="translate(0 {i * cellSize})">
								...
								<text font-size={cellSize / 2} fill="currentColor" stroke="none" text-anchor="end" x={padding - 2} y={cellSize - 2}>{i}</text>
							</g>
						{/each}
						</g>
					</g>
		...
		<g transform="translate({padding} {padding})">
			...
			<svg 
				   ...
				   on:mousemove={(e) => { if(isMouseDown) { handleCanvas(e) } }}>
				<g fill="currentColor" stroke="currentColor">
					{#each cells as {column, row}}
						<rect x={column * cellSize} y={row * cellSize} width="{cellSize}" height="{cellSize}" />
					{/each}
				</g>
